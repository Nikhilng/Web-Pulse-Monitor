...
subjects = ["", "Tom Cruise", "Cameron Diaz","RS"]
...
def detect_face(img):
    ...
    #result is a list of faces-- detect all faces in the image
    ...
    #extract the face area-- x,y, width,height
    (x, y, w, h) = faces[0]
...
face_recognizer = cv2.face.createLBPHFaceRecognizer_create()
...
#convert labels vector to numpy;
...
def predict(test_img):
    ...
    label, confidence = face_recognizer.predict(face)   #returns a label
...
#display both images
cv2.imshow('img1', cv2.resize(predicted_img1, (400, 500)))
